...
let comment_count = parseInt(parent.document.getElementById("comment_count").innerText);
let image_id;
let username;
let scroll;
...
onload = function () {
    ...
    image_id = parent.parent.document.baseURI.split("image_id=")[1];
    check_user();
    if(parseInt(parent.document.getElementById("comment_count").innerText) > 0)
        comment_fill();
};
...
function comment_send (){
    ...
    if (message) {
        ...
        comment_count += 1;
        parent.document.getElementById("comment_count").innerText = comment_count.toString();
        ...
        if (parseInt(parent.document.getElementById("comment_count").innerText) == 10)
            parent.document.getElementById("comment_count").style.marginLeft = "-35px";
    }
}
...
comment_text_input.addEventListener("keydown", function (e) {
    ...
    if(e.keyCode == 27) {
        parent.document.getElementById("close_button").click();
    }
});
